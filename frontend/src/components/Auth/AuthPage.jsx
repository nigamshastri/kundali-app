...
export default function AuthPage() {
  ...
  const submitRegister = async () => {
    ...
    try {
      ...
      setMode("verify-otp");
      setSuccess("OTP ઇ-મેઇલ પર મોકલ્યો!");
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
        ...
        {mode !== "verify-otp" && (
          <div style={S.tabs}>
            {["login", "register"].map((t) => (
              <button key={t} style={{ ...S.tab, ...(mode === t ? S.tabActive : {}) }}
        ...
        {mode === "verify-otp" && (
          <div>
            <div style={S.otpHeader}>
              ...
              <div style={S.otpSub}>
                <b style={{ color: "#ff6b1a" }}>{pendingEmail}</b><br />
                પર 6-અંકનો OTP મોકલ્યો છે
              </div>
            </div>
            ...
            <div style={S.otpFooter}>
              OTP ન આવ્યો?{" "}
              <button style={S.linkBtn} onClick={resendOtp} disabled={resending}>
                {resending ? "⏳..." : "ફરી મોકલો"}
              </button>
              {" · "}
