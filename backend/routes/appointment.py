...
def book():
    ...
    data = request.json
    # Check slot conflict
    conflict = appts_col.find_one({
        "date": data.get("date"),
        "time": data.get("time"),
        "status": {"$nin": ["cancelled"]}
    })
    if conflict:
        return jsonify({"error": "આ સ્લૉટ ભરાઈ ગઈ છે. બીજો સમય પસંદ કરો."}), 409
